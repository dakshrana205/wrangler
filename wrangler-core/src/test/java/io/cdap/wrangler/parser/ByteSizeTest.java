...
public class ByteSizeTest {
  ...
  public void testValidByteSizes() {
    ...
    Assert.assertEquals(1024L * 1024 * 1024 * 1024 * 1024, new ByteSize("1PB").getBytes());

    // Test decimal values
    Assert.assertEquals(512, new ByteSize("0.5KB").getBytes());
    Assert.assertEquals(1536, new ByteSize("1.5KB").getBytes());
    Assert.assertEquals(1024 * 1024 * 1.5, new ByteSize("1.5MB").getBytes(), 0.001);
  ...
  public void testValueAndType() {
    ...
    Assert.assertEquals(1024L, size.value());
    Assert.assertEquals("1KB", size.getOriginal());
  }
} 
